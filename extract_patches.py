# -*- coding: utf-8 -*-
"""
Created on Wed Jun 26 09:44:02 2019

@author: Ryan

Modified slightly from https://github.com/RyanCodes44/Machine-Learning-Exercise/blob/master/extract_patches.py
"""
...
def extract_patches(train_img,train_labels,n_dim,data_augmentation=False):
    ...
    for i in range(len(lab_patches)):
        ...
        l[np.nonzero(l)] = 1
        labels.append(l[:,0]) #RGB values are all the same, need pixel-wise
    
    return np.array(img_patches),np.round(np.array(np.squeeze(labels)))
